...
Visual Pre-Training techniques can be mainly divided into three categories: **supervised**, **unsupervised**, and **cross-modal** pretraining. The development of supervised pretraining is relatively mature and clear. The ImageNet dataset with about 15 million data scale, which laid the foundation for deep learning, is still the most commonly used dataset and has largely propelled the development of supervised visual pretraining. Moreover, these sort of image-level classification data introduces less representation bias and is thereby more favorable for downstream domain adaptations. Although many datasets nowadays already provide very fine pixel-level annotation, there is still a large amount of semantic information that cannot be annotated. More importantly, even if sufficient human cost is provided, it is difficult for us to define a standardized set of criteria to complete the annotation of all human-recognizable semantic information. Even if some semantic information has reached a very fine sub-pixel level, it can be recognized from the image by some means such as language, inscription, etc., if humans want to.
